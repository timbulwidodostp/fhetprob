...
{phang}
{bf:fhetprob} postestimation {hline 2} Postestimation tools for fhetprob{p_end}

{marker description}{...}
{title:Description}

{pstd}
The following postestimation commands are available after {cmd:fhetprob}:
...
{synopt :{helpb fhetprob postestimation##predict:predict}}predictions, residuals, influence statistics, and other diagnostic measures{p_end}
...
{cmd:fhetprob}.  It modifies the calculations made by {cmd:predict} so that
...
{pstd}Setup{p_end}
{phang2}{cmd:. use meap94_98}{p_end}

{pstd}Fit heteroskedastic probit model{p_end}
{phang2}{cmd:. fhetprob math4 lenrol, het(lenrol)}{p_end}
